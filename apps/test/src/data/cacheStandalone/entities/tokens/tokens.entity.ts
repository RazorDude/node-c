...
export interface DataCacheStandaloneToken extends RedisEntity<string> {
  refreshToken?: string;
  token: string;
  type: DataCacheStandaloneTokenType;
  userId: string;
}

const baseSchema = getDefaultEntitySchema(EntitySchemaColumnType.UUIDV4, 'token');
export const DataCacheStandaloneTokenSchema: EntitySchema = {
  ...baseSchema,
  columns: {
